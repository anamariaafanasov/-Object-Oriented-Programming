...
Player3::Player3(int x, int y, int tr):Player(x,y,tr)
{
...
coordonate Player3::get_next_position(map_matrix *m)
{
    // functie ce determina urmatoarea pozitie pe care se poate deplasa player3
    //pozitia e de tip coordonate;
    //deplasare:stanga,jos,dreapta,sus,diagonale:dreapta-jos,stanga-sus,dreapta-jos,stanga-sus
   ...
   coordonate next_position;
	...
	if((column+1<=n-1)&&(moved==0)) 	//dreapta
	{
		if(((*m)[row][column+1]==0) || ((*m)[row][column+1]==5))
		{
			next_position.j=column+1;
	...
	if(moved == 0)//paraseste harta fara comoara
    {
        cout<<"No more valid moves for Player3"<<endl;
        next_position.i=-2;
        next_position.j=-2;
    }
